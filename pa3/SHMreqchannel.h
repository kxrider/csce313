...
public:
	...
	int my_shm_send(void* msg, int len) {
		sem_wait(rd); // wait for reader to be done
		memcpy(segment, msg, len);
		sem_post(sd); // sender has just sent something
		return len;
	}

	int my_shm_recv(void* msg, int len) {
		sem_wait(sd); // wait for sender to finish sending
		memcpy(msg, segment, len);
		sem_post(rd); // receiver is done receiving
		return len;
	}
