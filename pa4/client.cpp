...
void file_thread_function(string fileName, int m, FIFORequestChannel* control, BoundedBuffer* reqBuffer) {
...
void worker_thread_function(BoundedBuffer* reqb, FIFORequestChannel* chan, BoundedBuffer* resb, int bufcap){
...
int main(int argc, char *argv[])
{
    ...
    while ((c = getopt(argc, argv, "p:m:f:b:w:n:h:")) != -1) {
        switch (c) {
            case 'p':
                p = atoi(optarg);
                break;
    ...
    if (pid == 0){
        ...
        char* argv[] = {"./server", "-m", (char*) to_string(m).c_str(), nullptr};
        execvp (argv[0], argv);
    }
    ...
    cout << "Took " << secs + usecs << " micro seconds" << endl;
